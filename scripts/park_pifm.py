...
def read_park_pifm_tiff(filepath):
    ...
    # ── 스캔 크기 및 원점 (tag50435) ──────────────────────────
    scan_um     = struct.unpack_from('<d', raw35, 140)[0]   # μm
    x_center_um = struct.unpack_from('<d', raw35, 164)[0]
    y_center_um = struct.unpack_from('<d', raw35, 156)[0]
    x_origin_um = x_center_um - scan_um / 2
    y_origin_um = y_center_um - scan_um / 2
    scan_x_nm   = scan_um * 1e3
    scan_y_nm   = scan_um * 1e3
    ...
    # v2[i] = X_stage, v0[i+1] = Y_stage (1칸 밀림)
    ...
    for i in range(n_pts):
        x_nm = (v2s[i]   - y_origin_um) * 1e3
        y_nm = (v0s[i+1] - x_origin_um) * 1e3
